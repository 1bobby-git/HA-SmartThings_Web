from pathlib import Path


def replace_once(path: str, old: str, new: str) -> None:
    target = Path(path)
    text = target.read_text(encoding="utf-8")
    if text.count(old) != 1:
        raise SystemExit(f"{path}: expected one match for {old!r}")
    target.write_text(text.replace(old, new, 1), encoding="utf-8")


replace_once(
    "README.md",
    "- 저장소 설치 앱 ID: `d55cafb9_smartthings_web_bridge`\n- 저장소 설치 내부 DNS: `d55cafb9-smartthings-web-bridge`",
    "- 현재 저장소 설치 앱 ID: `8a97f131_smartthings_web_bridge`\n- 현재 저장소 설치 내부 DNS: `8a97f131-smartthings-web-bridge`",
)
replace_once(
    "tools/update-local-addon.sh",
    "저장소 앱 ID가 d55cafb9_smartthings_web_bridge라면 local 주소가 아니라 저장소 주소를 사용해야 합니다.",
    "저장소 앱 ID가 8a97f131_smartthings_web_bridge라면 local 주소가 아니라 현재 저장소 주소를 사용해야 합니다.",
)
replace_once(
    "docs/local-addon-update.md",
    "- 앱 ID가 `d55cafb9_smartthings_web_bridge`이면 저장소 설치 앱입니다. 이 경우 앱 스토어에서 0.1.169로 업데이트하며 내부 주소는 `http://d55cafb9-smartthings-web-bridge:8100`입니다.\n- 통합 0.1.169는 `local`과 `d55cafb9` 두 내부 주소를 안전하게 확인한 뒤 실제 응답한 주소를 구성 항목에 저장합니다.",
    "- 현재 저장소 설치 앱 ID는 `8a97f131_smartthings_web_bridge`이고 내부 주소는 `http://8a97f131-smartthings-web-bridge:8100`입니다. 저장소 설치 앱은 앱 스토어에서 최신 버전으로 업데이트합니다.\n- 이전 `d55cafb9` 주소는 기존 구성 복구용 후보로만 유지합니다.\n- 통합 0.1.179부터 Supervisor가 전달한 실제 앱 slug로 Bridge 주소를 자동 입력하며, `/addons` 로컬 설치에서는 `http://local-smartthings-web-bridge:8100`을 사용합니다.",
)

test_path = Path("tests/bridge-autodiscovery-contract.test.ts")
test = test_path.read_text(encoding="utf-8")
old = '''    const readme = readText("README.md");

    expect(constants).toContain(
'''
new = '''    const readme = readText("README.md");
    const localUpdateDocs = readText("docs/local-addon-update.md");
    const localUpdateHelper = readText("tools/update-local-addon.sh");

    expect(constants).toContain(
'''
if test.count(old) != 1:
    raise SystemExit("contract test declaration anchor mismatch")
test = test.replace(old, new, 1)
old = '''    expect(readme).toContain("칸에 자동 입력");
  });
});
'''
new = '''    expect(readme).toContain("칸에 자동 입력");
    expect(readme).toContain(
      "현재 저장소 설치 앱 ID: `8a97f131_smartthings_web_bridge`"
    );
    expect(readme).toContain(
      "현재 저장소 설치 내부 DNS: `8a97f131-smartthings-web-bridge`"
    );
    expect(localUpdateDocs).toContain(
      "현재 저장소 설치 앱 ID는 `8a97f131_smartthings_web_bridge`"
    );
    expect(localUpdateHelper).toContain(
      "저장소 앱 ID가 8a97f131_smartthings_web_bridge라면"
    );
  });
});
'''
if test.count(old) != 1:
    raise SystemExit("contract test assertion anchor mismatch")
test_path.write_text(test.replace(old, new, 1), encoding="utf-8")
