...
## 1.8.16 재실 센서 및 방/영역 동기화

카운터의 실제 상태/명령을 근거로 기본 모델을 **재실 센서 (인원 카운터)**로 표시합니다. 실제 `occupancy`/`peopleCounter` 상태에서 **읽기 전용 재실(occupancy)** 엔티티를 생성하며 기존 number 설정값을 실제 상태로 사용하지 않습니다. 기존 인원수·숫자 제어·ID는 유지합니다.

**Bridge 앱과 HACS 통합을 모두 1.8.16으로 업데이트하고 재시작하세요.** 방 동기화는 기본 켬이며 확인된 Advanced room과 같은 location의 방 목록으로 기존 잘못된 기기 영역도 교정합니다. 이미 저장한 `sync_rooms: false`는 그대로이므로 이 경우 통합 옵션에서 **SmartThings 방과 기기 영역 동기화**를 켜야 합니다. 동기화 모드에서는 수동 기기 영역보다 SmartThings 방을 우선합니다. 엔티티별 영역과 다른 통합 공유 기기는 보존합니다. [동작 조건·예제·검증 한계](docs/OCCUPANCY_ROOM_SYNC_1.8.16.md).
...
## Home Monitor (1.8.16에서도 기존 직접 제어 유지)
...
> **현재 버전: Bridge `1.8.16` / HA 통합 `1.8.16` · 기존 실환경 부분 검증, 새 개선분은 CI 검증**
>
...
최초 1.8.7 배포는 Home Assistant 통합만 업데이트했습니다. 해당 최초 배포에서 브리지 앱과 Node 패키지는 1.8.6을 유지했습니다. 함께 첨부한 smartthings-web-bridge-1.8.6.tgz는 이전 릴리스와 SHA-256이 동일한 파일이며 재빌드하거나 업데이트한 앱이 아닙니다. HACS 통합 업데이트를 위해 브리지를 재설치할 필요는 없습니다. 브리지 1.8.7의 후속 변경은 `bridge-v1.8.7`에 반영됐습니다.
