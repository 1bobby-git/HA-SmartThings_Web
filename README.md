...
## 1.8.15 방/영역 동기화 및 카운터 표시

Advanced 방 정보를 Web 카드가 덮어쓰는 경로를 수정했습니다. **이미 잘못 지정된 영역도 보정하려면**, Bridge 앱과 HACS 통합을 모두 1.8.15로 업데이트한 후 통합 옵션의 **SmartThings 방과 기기 영역 동기화**를 켜세요. 기본값은 기존 사용자 영역 보호를 위해 꺼져 있습니다. 엔티티별 수동 영역은 변경하지 않습니다. 카운터의 실제 상태/명령이 확인되면 모델명을 단순 모션 아이콘 분류로 표시하지 않습니다. [동작 조건·유형 구분·검증 한계](docs/ROOM_SYNC_1.8.15.md)를 참조하세요.
...
## Home Monitor (1.8.15에서도 기존 직접 제어 유지)
...
> **현재 버전: Bridge `1.8.15` / HA 통합 `1.8.15` · 기존 실환경 부분 검증, 새 개선분은 CI 검증**
>
