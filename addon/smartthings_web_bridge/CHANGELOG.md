...
- 사용자 보고상 1.8.4에서 외출·해제가 변경되지만 수십 초에서 수 분 뒤 반영되는 증상을 기준으로 최근 1.8.3~1.8.5 변경을 검토했습니다. 실제 동작한 직접 보안 버튼과 1.8.5의 확인 완료까지 탭 유지 처리를 보존합니다.
- Advanced 위치 목록과 상태 없는 Web 위치 응답이 기존 armState/updatedAt을 지우던 결함을 수정했습니다. 오래되거나 날짜 없는 반대 상태는 최신 보안 이벤트를 덮어쓰지 않습니다.
- Home Assistant가 이미 지원하는 AWAY/STAY/OFF 및 armedaway/armedstay 표현을 명령 확인에도 일관되게 적용합니다. ARMING/PENDING/알 수 없는 값이나 클릭만으로 성공을 판정하지 않습니다.
- 정확한 동일 위치의 열린 keeper 대시보드는 실제 Playwright 클릭으로 재사용합니다. 다른 위치의 keeper는 조작하지 않으며 새 탭이 필요하면 알려진 원본 위치로 직접 이동합니다. 준비된 화면에서 고정 대기나 전체 페이지 재로드를 하지 않습니다.
- 보안 상태 재확인은 전체 Advanced 기기 인벤토리 대신 기존 인증 Cake 클라이언트의 읽기 전용 api/location get을 사용합니다. 정확한 위치와 허용된 상태만 수용하며, 클라이언트 부재/잘못된 응답/기한 초과는 성공으로 처리하지 않습니다. 새 인증 연결이나 보안 상태 쓰기 API는 만들지 않습니다.
- 보안 요청의 서비스/UI 대기열은 각각 10초 안에 시작하지 못한 작업을 command_queue_timeout으로 종료하고 이후 실행하지 않습니다. 이미 실행 중인 작업의 순서와 정리는 유지합니다. 새 페이지의 반복 제어 탐색에는 공통 시간 예산을 적용합니다.
- home_monitor_queue_ms_*, home_monitor_dispatch_ms_* 및 상태 재조회 진단을 추가했습니다. 계정·장소명·원본 ID·쿠키는 기록하지 않습니다.
- 상태 보존/별칭 확인/만료된 요청의 미실행을 검증하는 회귀 테스트와 실제 Chromium 합성 화면의 연속 외출·해제·실내 제어를 추가했습니다. 합성 성능 측정은 사용자 HAOS/Samsung 계정의 실제 지연 보장이 아닙니다. Scene·로그인·엔티티 ID·일반 기기 이벤트 폭주 억제 경로는 유지합니다.
...
- Bridge SSE 전송에 75ms 인벤토리 이벤트 병합과 128개 제한 백프레셔 큐를 적용하고, 큐가 포화되면 최신 인벤토리 복구 마커 하나로 축약합니다.
- 전체 스냅샷에서 값은 같고 타임스탬프만 바뀐 상태·헬스 정보를 저장하되 인벤토리 이벤트로 다시 발행하지 않습니다.
- Home Assistant는 전체 인벤토리를 받아도 실제 값이 달라진 상태 리스너만 갱신하며, 온라인 상태·토폴로지·메타데이터 변경에만 장치 및 전역 discovery/registry 콜백을 실행합니다.
- 300개 장치 capability catalog, 타임스탬프 전용 스냅샷, SSE 병합·순서·큐 포화, HA 리스너 범위를 검증하는 회귀 테스트를 추가했습니다.

## 0.1.174

- 센서가 실시간 값 타입 변화에 따라 `device_class`와 `state_class`를 반복 변경하던 동작을 제거해 Home Assistant 엔티티 레지스트리와 WebSocket 이벤트 폭주를 차단합니다.
- 숫자 센서는 생성 시 확정된 capability 메타데이터를 유지하고, 이후 비숫자 오류값은 상태 메타데이터를 뒤집는 대신 `unknown`으로 처리합니다.
- 0.1.173의 중복 상태·SQLite·Chromium 캐시 쓰기 억제와 0.1.171의 레지스트리 backpressure 보호를 함께 포함합니다.
...
- ExampleOffice 대시보드의 React 렌더링이 끝날 때까지 `SmartThings Home Monitor` 카드와 `보안(외출)`·`보안(실내)` 컨트롤을 기다리고, 접근성 역할이 없는 경우에도 카드 내부의 정확한 표시 문자열만 제한적으로 클릭합니다.
...
- 구성 항목 시작 시 선택된 Location만 비어 있으면 Advanced 인벤토리를 한 번 자동 재조회하여 ExampleOffice 기기를 복구하도록 보강했습니다.
- 0.1.166의 ExampleOffice Home Monitor 직접 제어 동작은 그대로 유지합니다.

## 0.1.166

- 현재 브라우저 URL의 raw Location ID가 명령 대상과 일치하면 별칭 정규화나 Location picker 탐색 전에 즉시 현재 위치로 확정합니다.
- ExampleOffice 실제 대시보드의 SmartThings Home Monitor 카드 안에 노출되는 `보안(외출)` 및 `보안(실내)` 버튼을 카드 범위에서 직접 찾아 제어합니다.
...
- Home과 ExampleOffice를 포함한 모든 Location Home Monitor는 SmartThings 웹 세션으로 인증하며 HA 키패드 코드를 요구하지 않습니다.
...
- 다중 Location에서 ExampleOffice처럼 첫 SECURITY_ARM_STATE_EVENT가 아직 수신되지 않은 Home Monitor도 unavailable 대신 unknown 상태로 유지하고 외출/재실/해제 명령을 허용합니다.
