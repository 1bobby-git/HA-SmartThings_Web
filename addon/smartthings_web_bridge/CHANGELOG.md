...
- Chromium 프로필을 기본으로 유지하면서, bridge-secret으로 암호화한 세션 상태 백업을 `/data/session-state.json`에 원자적으로 저장합니다. 인증 probe가 실제 재로그인을 확인할 때만 백업을 복원하며, 쿠키·localStorage 원문은 로그에 남기지 않습니다.
