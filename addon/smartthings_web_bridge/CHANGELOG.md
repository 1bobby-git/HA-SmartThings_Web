...
- 대표 HA switch 뒤에 둘 이상의 검증 가능한 switch component가 있으면 Advanced component transaction을 안정된 순서로 실행하고 bounded confirmation window의 조기·최종 `/status` 조회에서 전체 벡터가 일치할 때만 성공으로 확인합니다.
