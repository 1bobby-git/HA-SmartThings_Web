...
## 조명 묶음 전송 (1.8.32, 선택 사항)

`light_command_batch_enabled` 기본값은 `false`입니다. 기존 조명 전송 방식을 유지합니다. 여러 명령의 순차 접수 대기를 줄여 시험하려면 앱 구성에서 이 값을 `true`로 변경하고 Bridge 앱을 재시작하세요. 다른 설정은 유지하세요.

```yaml
light_command_batch_enabled: true
```

같은 전구/컴포넌트의 검증된 전원·밝기·색상 명령을 한 POST로 보내며 실제 상태 확인은 생략하지 않습니다. 기기의 원자적/순차 실행을 보장하지 않으며 부분 실패를 자동 재전송하지 않습니다. 해당 전구에서 실패하거나 일부 값이 적용되지 않으면 `false`로 되돌리고 앱을 재시작하세요. 단독 켜기/끄기의 서버 왕복 시간은 없어지지 않습니다.

## SmartThings 로그인 유지 자동 적용 (1.8.49)

`keep_signed_in_enabled: true` (기본값)는 브릿지의 **전용 Chromium**에서만 SmartThings 웹 설정의 `로그인 유지`를 켭니다. 휴대폰 브라우저의 설정을 가져오거나 삼성 계정의 보안 정책을 변경하는 기능이 아닙니다. 공유 PC의 로그인 설정에는 적용하지 않습니다.

처음 로그인하거나 Bridge 앱을 재시작하면 먼저 실제 인증을 검사하고 별도 후보 탭에서 설정을 확인합니다. 후보를 새로 열어 설정 저장 여부와 인증을 확인한 다음 운영 탭으로 전환합니다. 이미 켜져 있으면 다시 토글하지 않으며 꺼진 경우에만 한 번 켭니다. 지원 데이터 접근(`Account data access`), `세션 길이` 선택, `로그아웃`, 인증 입력은 조작하지 않습니다.

브릿지 카드에서 `로그인 유지 켜짐 확인`과 현재 `브라우저 세션`을 각각 확인하세요. 카드 내용은 `상태 다시 확인`으로 다시 읽을 수 있습니다. `설정 확인 필요`라면 noVNC의 실제 SmartThings 설정에서 직접 확인하세요. 새 웹 UI를 판독하지 못하면 추측해 클릭하지 않습니다.

자동 적용을 중단하려면 앱 구성에서 `keep_signed_in_enabled`를 `false`로 저장하고 재시작하세요. 이 옵션은 자동 적용만 끕니다. 이미 저장된 웹의 로그인 유지 설정은 자동으로 끄지 않으므로 필요하면 웹 설정에서 직접 끄세요. 브라우저 실행 중 로그인 유지 시간을 늘리는 설정이므로 전용 브릿지와 Home Assistant 관리 접근은 신뢰할 수 있는 사용자로 제한하세요.

실제 계정에서 8시간·24시간 이후에도 유지되는지는 별도 실사용 검증 대상입니다. 설정이 켜져 있어도 서버가 재인증을 요구할 수 있으며, 브릿지 재시작 후 실제 인증 확인을 생략하지 않습니다.
