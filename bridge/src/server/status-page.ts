...
function renderOverallPanel(report: HealthReport, state: OverallState): string {
  const title = state === "ready" ? "브릿지가 준비되었습니다" : state === "starting" ? "브릿지를 준비하고 있습니다" : "브릿지 확인이 필요합니다";
  const copy = state === "ready"
    ? "SmartThings 관찰과 Home Assistant 연결에 필요한 조건이 모두 충족되었습니다."
    : state === "starting"
      ? "브릿지 서비스는 실행 중이며 일부 준비 상태 확인을 마치는 중입니다."
      : "브릿지가 현재 정상 상태가 아닙니다. 브라우저 로그인 또는 상세 진단에서 원인을 확인하세요.";
  ...
  return `<section class="hc-hero" data-overall-state="${state}" aria-labelledby="bridge-state-heading">
    <div class="hc-hero-grid">
      ...
      <ul class="hc-status-list" aria-label="브릿지 준비 상태 요약">
        <li class="hc-status-item"><span class="hc-status-label">서비스</span><strong class="hc-status-value">${report.live ? "정상" : "오프라인"}</strong></li>
        <li class="hc-status-item"><span class="hc-status-label">HA 준비 상태</span><strong class="hc-status-value">${report.ready ? "준비됨" : "준비 중"}</strong></li>
        <li class="hc-status-item"><span class="hc-status-label">실행 상태</span><strong class="hc-status-value">${escapeHtml(formatRuntimeState(report.details.state))}</strong></li>
      </ul>
    </div>
  </section>`;
}
