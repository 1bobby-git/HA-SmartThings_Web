...
describe("SmartThingsWebUiCommandExecutor", () => {
  ...
  test("skips the matching ExampleOffice URL picker and clicks both dashboard Home Monitor controls", async () => {
      ...
      page.getByRole = vi.fn(
        (role: string, options?: { name?: string | RegExp }) => {
          if (
            role === "button" &&
            options?.name instanceof RegExp &&
            options.name.test("ExampleOffice")
          ) {
            pickerQueries += 1;
          }
          return missing;
      ...
      const executor = new SmartThingsWebUiCommandExecutor(
        ...
        },
        {
          resolveRawLocationId: (alias) =>
            alias === "loc_office" ? rawLocationId : undefined
        }
      );
