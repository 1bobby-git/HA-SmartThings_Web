...
describe("SafeCommandService", () => {
  ...
  describe("Exact switch status corroboration regression", () => {
    ...
    const switchBody = (value: string, timestamp: string | null) => ({ items: [{
      deviceId: "dev_001", locationId: "loc_001", label: "Regression switch", type: "switch",
      status: { components: { main: { identifier_switch: { switch: { value, timestamp } } } } }
    }] });
    const switchRequest = (command = "off") => ({ targetType: "device", targetId: "dev_001",
      component: "main", capability: "identifier_switch", attribute: "switch", command,
    ...
    test.each([null, switchTimestamp])("confirms same-value GETs with %s timestamps without retransmitting", async (timestamp) => {
      ...
      try {
        ...
        const resync = vi.fn(async (request?: import("../../src/command/command-service.js").CommandResyncRequest) => {
          ...
          const observedStates = await readLightCommandStatus(store, "dev_001", "loc_001",
            read, request?.lightComponent, request?.switchTarget);
          return { source: "advanced_device_status" as const, deviceId: "dev_001", locationId: "loc_001",
        ...
        expect(resync).toHaveBeenCalledWith({ deviceId: "dev_001", switchTarget: { component: "main", capability: "identifier_switch" } });
        expect(store.commandState("dev_001", "loc_001", "main", "identifier_switch", "switch"))
