...
def _canonical_generated_state_entity_id(
    ...
    if (
        ...
        object_id = canonical_entity_object_id(inventory, device, "presence")
        if not object_id:
            return None
        target = f"{domain}.{object_id}"
        return target if target != getattr(entity_entry, "entity_id", "") else None
...
def _generated_presence_state_entity_id(
    ...
    if not current_object_id:
        return False
    canonical_presence = canonical_entity_object_id(inventory, device, "presence")
    generated = {canonical_presence} if canonical_presence else set()
    for metadata_name in (
        getattr(entity_entry, "original_name", None),
        getattr(entity_entry, "object_id_base", None),
        ...
        if not metadata_slug.startswith(f"{device_object_id}_"):
            generated.add(f"{device_object_id}_{metadata_slug}")
    if current_object_id in generated:
        return True
    for candidate in generated:
        prefix = f"{candidate}_"
        if not current_object_id.startswith(prefix):
            continue
        suffix = current_object_id[len(prefix) :]
        if suffix.isdigit() and int(suffix) >= 2:
            return True
    return False
