...
- bounded confirmation window의 독립된 조기·최종 Advanced `/status` 조회 중 `advanced_device_status` source와 모든 component 값이 함께 일치해야 성공한다. Location event나 전체 inventory evidence만으로는 확정하지 않는다.
...
- full Vitest: 68 files, 885 tests passed
- Home Assistant Python unittest discovery: 235 tests passed
- `npm run typecheck`: passed
- `npm run build`: passed
- `npm run package:addon`: passed
- add-on package manifest SHA-256: `4e6ac9c1359f82d02932e75a85fd243a5664ca8409042b83ea1521fe43bc21c2`
...
- 조기 status read가 끝나지 않아도 final status read와 전체 confirmation timeout은 독립적으로 진행되어 장치 queue가 무기한 막히지 않는다.
- `CommandResyncEvidence.source`를 명시하고 component confirmation은 `advanced_device_status`만 수용한다.
