...
Version 0.1.26 is implemented and packaged locally but has not been deployed to HAOS. It adds one bounded in-memory physical-action correlation window, safe fixed presets, dedupe-aware candidates, exact keeper-page isolation, and fixed Ingress controls without adding commands, DOM state scraping, direct SmartThings API calls, Home Assistant entities, or a persistent event journal. Physical-action correlation remains unverified until the active 0.1.25 soak is sealed, 0.1.26 is deployed without disturbing that evidence, and one real safe user action produces one unique passing result.
