...
Version 0.1.27 is deployed. A Bridge-only restart restored `CONNECTED` state and 213 devices after the local sequence reset while Home Assistant Core remained running, and subsequent live sensor changes reached Home Assistant. The physical-action probe still fails closed on component-less unrelated events, so manual attribution remains unverified.
