...
describe("Home Assistant add-on metadata", () => {
  test("publishes identifier-free toggle diagnostics as version 0.1.60", () => {
    ...
    expect(packageMetadata.version).toBe("0.1.60");
    expect(protocolMetadata.bridge_version).toBe("0.1.60");
    expect(protocolMetadata.protocol_version).toBe(4);
    expect(runtime).toContain('const bridgeVersion = "0.1.60";');
    expect(changelog).toContain("## 0.1.60");
    expect(changelog).toContain("identifier-free toggle discovery stages");
