...
describe("Home Assistant add-on metadata", () => {
  test("publishes session-prune dedupe as version 0.1.106", () => {
    ...
    expect(config.version).toBe("0.1.106");
    expect(packageMetadata.version).toBe("0.1.106");
    expect(protocolMetadata.bridge_version).toBe("0.1.106");
    expect(protocolMetadata.protocol_version).toBe(4);
    expect(runtime).toContain('const bridgeVersion = "0.1.106";');
