...
describe("Home Assistant add-on metadata", () => {
  test("publishes the live fan and media control repair as version 0.1.47", () => {
    ...
    expect(config.version).toBe("0.1.47");
    expect(packageMetadata.version).toBe("0.1.47");
    expect(protocolMetadata.bridge_version).toBe("0.1.47");
    expect(protocolMetadata.protocol_version).toBe(3);
    expect(runtime).toContain('const bridgeVersion = "0.1.47";');
    expect(changelog).toContain("## 0.1.47");
