...
describe("Home Assistant add-on metadata", () => {
  test("publishes low-latency background delivery as version 0.1.86", () => {
    ...
    expect(config.version).toBe("0.1.86");
    expect(packageMetadata.version).toBe("0.1.86");
    expect(protocolMetadata.bridge_version).toBe("0.1.86");
    expect(protocolMetadata.protocol_version).toBe(4);
    expect(runtime).toContain('const bridgeVersion = "0.1.86";');
    expect(changelog).toContain("## 0.1.86");
