...
describe("Home Assistant add-on metadata", () => {
  test("publishes the push-confirmed command path as version 0.1.29", () => {
    ...
    expect(config.version).toBe("0.1.29");
    expect(packageMetadata.version).toBe("0.1.29");
    expect(protocolMetadata.bridge_version).toBe("0.1.29");
    expect(protocolMetadata.protocol_version).toBe(1);
    expect(runtime).toContain('const bridgeVersion = "0.1.29";');
