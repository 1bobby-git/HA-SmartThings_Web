...
describe("Home Assistant add-on metadata", () => {
  test("packages Advanced command and entity-ID parity as version 0.1.171", () => {
    ...
    expect(config.version).toBe("0.1.171");
    expect(config.homeassistant_api).toBe(true);
    expect(packageMetadata.version).toBe("0.1.171");
    expect(protocolMetadata.bridge_version).toBe("0.1.171");
    expect(protocolMetadata.protocol_version).toBe(5);
    expect(runtime).toContain('const bridgeVersion = "0.1.171";');
    expect(changelog).toContain("## 0.1.171");
