...
describe("Home Assistant add-on metadata", () => {
  test("publishes exact accessible detail-heading readiness as version 0.1.65", () => {
    ...
    expect(config.version).toBe("0.1.65");
    expect(packageMetadata.version).toBe("0.1.65");
    expect(protocolMetadata.bridge_version).toBe("0.1.65");
    expect(protocolMetadata.protocol_version).toBe(4);
    expect(runtime).toContain('const bridgeVersion = "0.1.65";');
    expect(changelog).toContain("## 0.1.65");
