...
describe("Home Assistant add-on metadata", () => {
  test("packages the stable restored entity IDs as version 0.1.121", () => {
    ...
    expect(config.version).toBe("0.1.121");
    expect(config.homeassistant_api).toBe(true);
    expect(packageMetadata.version).toBe("0.1.121");
    expect(protocolMetadata.bridge_version).toBe("0.1.121");
    expect(protocolMetadata.protocol_version).toBe(4);
    expect(runtime).toContain('const bridgeVersion = "0.1.121";');
    expect(runtime).not.toContain("confirmationStabilityMs: 500");
    expect(changelog).toContain("## 0.1.121");
