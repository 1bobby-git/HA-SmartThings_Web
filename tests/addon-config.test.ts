...
describe("Home Assistant add-on metadata", () => {
  test("publishes authenticated in-page command delivery as version 0.1.85", () => {
    ...
    expect(config.version).toBe("0.1.85");
    expect(packageMetadata.version).toBe("0.1.85");
    expect(protocolMetadata.bridge_version).toBe("0.1.85");
    expect(protocolMetadata.protocol_version).toBe(4);
    expect(runtime).toContain('const bridgeVersion = "0.1.85";');
    expect(changelog).toContain("## 0.1.85");
