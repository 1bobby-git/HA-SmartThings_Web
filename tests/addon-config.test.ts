...
describe("Home Assistant add-on metadata", () => {
  test("packages restored entity metadata repair as version 0.1.128", () => {
    ...
    expect(config.version).toBe("0.1.128");
    expect(config.homeassistant_api).toBe(true);
    expect(packageMetadata.version).toBe("0.1.128");
    expect(protocolMetadata.bridge_version).toBe("0.1.128");
    expect(protocolMetadata.protocol_version).toBe(4);
    expect(runtime).toContain('const bridgeVersion = "0.1.128";');
