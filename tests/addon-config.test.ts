...
describe("Home Assistant add-on metadata", () => {
  test("publishes the safe session and command latency repair as version 0.1.48", () => {
    ...
    expect(config.version).toBe("0.1.48");
    expect(packageMetadata.version).toBe("0.1.48");
    expect(protocolMetadata.bridge_version).toBe("0.1.48");
    expect(protocolMetadata.protocol_version).toBe(3);
    expect(runtime).toContain('const bridgeVersion = "0.1.48";');
    expect(changelog).toContain("## 0.1.48");
