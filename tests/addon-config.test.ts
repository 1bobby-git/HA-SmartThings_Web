...
describe("Home Assistant add-on metadata", () => {
  test("publishes non-waiting same-page warm-card recovery as version 0.1.76", () => {
    ...
    expect(config.version).toBe("0.1.76");
    expect(packageMetadata.version).toBe("0.1.76");
    expect(protocolMetadata.bridge_version).toBe("0.1.76");
    expect(protocolMetadata.protocol_version).toBe(4);
    expect(runtime).toContain('const bridgeVersion = "0.1.76";');
