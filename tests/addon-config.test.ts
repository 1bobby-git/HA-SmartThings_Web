...
describe("Home Assistant add-on metadata", () => {
  test("publishes push-listener isolation and room hydration as version 0.1.77", () => {
    ...
    expect(config.version).toBe("0.1.77");
    expect(packageMetadata.version).toBe("0.1.77");
    expect(protocolMetadata.bridge_version).toBe("0.1.77");
    expect(protocolMetadata.protocol_version).toBe(4);
    expect(runtime).toContain('const bridgeVersion = "0.1.77";');
    expect(changelog).toContain("## 0.1.77");
