...
describe("Home Assistant add-on metadata", () => {
  test("publishes the safe overview-opener repair as version 0.1.51", () => {
    ...
    expect(config.version).toBe("0.1.51");
    expect(packageMetadata.version).toBe("0.1.51");
    expect(protocolMetadata.bridge_version).toBe("0.1.51");
    expect(protocolMetadata.protocol_version).toBe(4);
    expect(runtime).toContain('const bridgeVersion = "0.1.51";');
    expect(changelog).toContain("## 0.1.51");
