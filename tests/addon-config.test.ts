...
describe("Home Assistant add-on metadata", () => {
  test("publishes detail-route readiness as version 0.1.59", () => {
    ...
    expect(config.version).toBe("0.1.59");
    expect(packageMetadata.version).toBe("0.1.59");
    expect(protocolMetadata.bridge_version).toBe("0.1.59");
    expect(protocolMetadata.protocol_version).toBe(4);
    expect(runtime).toContain('const bridgeVersion = "0.1.59";');
    expect(changelog).toContain("## 0.1.59");
