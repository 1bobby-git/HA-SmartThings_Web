...
describe("Home Assistant add-on metadata", () => {
  test("publishes bounded cold-navigation recovery as version 0.1.68", () => {
    ...
    expect(config.version).toBe("0.1.68");
    expect(packageMetadata.version).toBe("0.1.68");
    expect(protocolMetadata.bridge_version).toBe("0.1.68");
    expect(protocolMetadata.protocol_version).toBe(4);
    expect(runtime).toContain('const bridgeVersion = "0.1.68";');
    expect(changelog).toContain("## 0.1.68");
