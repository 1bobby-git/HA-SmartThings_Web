...
describe("Home Assistant add-on metadata", () => {
  test("packages raw observed camera thumbnail recovery as version 0.1.124", () => {
    ...
    expect(config.version).toBe("0.1.124");
    expect(config.homeassistant_api).toBe(true);
    expect(packageMetadata.version).toBe("0.1.124");
    expect(protocolMetadata.bridge_version).toBe("0.1.124");
    expect(protocolMetadata.protocol_version).toBe(4);
    expect(runtime).toContain('const bridgeVersion = "0.1.124";');
    expect(runtime).not.toContain("confirmationStabilityMs: 500");
    expect(changelog).toContain("## 0.1.124");
