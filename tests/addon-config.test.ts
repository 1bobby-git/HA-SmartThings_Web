...
describe("Home Assistant add-on metadata", () => {
  test("publishes dialog-scoped control and reconnect recovery as version 0.1.66", () => {
    ...
    expect(config.version).toBe("0.1.66");
    expect(packageMetadata.version).toBe("0.1.66");
    expect(protocolMetadata.bridge_version).toBe("0.1.66");
    expect(protocolMetadata.protocol_version).toBe(4);
    expect(runtime).toContain('const bridgeVersion = "0.1.66";');
    expect(changelog).toContain("## 0.1.66");
