...
describe("Home Assistant add-on metadata", () => {
  test("publishes SmartThings room-name-free device names as version 0.1.99", () => {
    ...
    expect(config.version).toBe("0.1.99");
    expect(packageMetadata.version).toBe("0.1.99");
    expect(protocolMetadata.bridge_version).toBe("0.1.99");
    expect(protocolMetadata.protocol_version).toBe(4);
    expect(runtime).toContain('const bridgeVersion = "0.1.99";');
