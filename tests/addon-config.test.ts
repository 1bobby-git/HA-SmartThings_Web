...
describe("Home Assistant add-on metadata", () => {
  test("publishes the layered-toggle repair as version 0.1.52", () => {
    ...
    expect(config.version).toBe("0.1.52");
    expect(packageMetadata.version).toBe("0.1.52");
    expect(protocolMetadata.bridge_version).toBe("0.1.52");
    expect(protocolMetadata.protocol_version).toBe(4);
    expect(runtime).toContain('const bridgeVersion = "0.1.52";');
