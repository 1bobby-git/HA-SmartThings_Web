...
describe("Home Assistant add-on metadata", () => {
  test("packages capability stability and storage-write optimization as version 0.1.174", () => {
    ...
    expect(config.version).toBe("0.1.174");
    expect(config.homeassistant_api).toBe(true);
    expect(packageMetadata.version).toBe("0.1.174");
    expect(protocolMetadata.bridge_version).toBe("0.1.174");
    expect(protocolMetadata.protocol_version).toBe(5);
    expect(runtime).toContain('const bridgeVersion = "0.1.174";');
