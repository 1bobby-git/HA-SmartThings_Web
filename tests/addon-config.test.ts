...
describe("Home Assistant add-on metadata", () => {
  test("publishes fail-closed device targeting as version 0.1.56", () => {
    ...
    expect(config.version).toBe("0.1.56");
    expect(packageMetadata.version).toBe("0.1.56");
    expect(protocolMetadata.bridge_version).toBe("0.1.56");
    expect(protocolMetadata.protocol_version).toBe(4);
    expect(runtime).toContain('const bridgeVersion = "0.1.56";');
    expect(changelog).toContain("## 0.1.56");
