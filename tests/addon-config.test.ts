...
describe("Home Assistant add-on metadata", () => {
  test("publishes scoped button toggles as version 0.1.58", () => {
    ...
    expect(config.version).toBe("0.1.58");
    expect(packageMetadata.version).toBe("0.1.58");
    expect(protocolMetadata.bridge_version).toBe("0.1.58");
    expect(protocolMetadata.protocol_version).toBe(4);
    expect(runtime).toContain('const bridgeVersion = "0.1.58";');
    expect(changelog).toContain("## 0.1.58");
