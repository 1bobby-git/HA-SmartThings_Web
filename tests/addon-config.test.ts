...
describe("Home Assistant add-on metadata", () => {
  test("publishes the camera correlation fix as version 0.1.34", () => {
    ...
    expect(config.version).toBe("0.1.34");
    expect(packageMetadata.version).toBe("0.1.34");
    expect(protocolMetadata.bridge_version).toBe("0.1.34");
    expect(protocolMetadata.protocol_version).toBe(1);
    expect(runtime).toContain('const bridgeVersion = "0.1.34";');
    expect(changelog).toContain("## 0.1.34");
