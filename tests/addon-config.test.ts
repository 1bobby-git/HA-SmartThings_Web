...
describe("Home Assistant add-on metadata", () => {
  test("publishes identifier-shape-safe repair as version 0.1.103", () => {
    ...
    expect(config.version).toBe("0.1.103");
    expect(packageMetadata.version).toBe("0.1.103");
    expect(protocolMetadata.bridge_version).toBe("0.1.103");
    expect(protocolMetadata.protocol_version).toBe(4);
    expect(runtime).toContain('const bridgeVersion = "0.1.103";');
