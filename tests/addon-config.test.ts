...
describe("Home Assistant add-on metadata", () => {
  test("publishes foreground command priority as version 0.1.55", () => {
    ...
    expect(config.version).toBe("0.1.55");
    expect(packageMetadata.version).toBe("0.1.55");
    expect(protocolMetadata.bridge_version).toBe("0.1.55");
    expect(protocolMetadata.protocol_version).toBe(4);
    expect(runtime).toContain('const bridgeVersion = "0.1.55";');
    expect(changelog).toContain("## 0.1.55");
