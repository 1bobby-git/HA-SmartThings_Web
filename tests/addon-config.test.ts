...
describe("Home Assistant add-on metadata", () => {
  test("publishes the exact device-opener repair as version 0.1.50", () => {
    ...
    expect(config.version).toBe("0.1.50");
    expect(packageMetadata.version).toBe("0.1.50");
    expect(protocolMetadata.bridge_version).toBe("0.1.50");
    expect(protocolMetadata.protocol_version).toBe(4);
    expect(runtime).toContain('const bridgeVersion = "0.1.50";');
    expect(changelog).toContain("## 0.1.50");
