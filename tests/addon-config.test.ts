...
describe("Home Assistant add-on metadata", () => {
  test("publishes cached inventory health recovery as version 0.1.78", () => {
    ...
    expect(config.version).toBe("0.1.78");
    expect(packageMetadata.version).toBe("0.1.78");
    expect(protocolMetadata.bridge_version).toBe("0.1.78");
    expect(protocolMetadata.protocol_version).toBe(4);
    expect(runtime).toContain('const bridgeVersion = "0.1.78";');
    expect(changelog).toContain("## 0.1.78");
    expect(changelog).toContain("restored cached inventory count");
