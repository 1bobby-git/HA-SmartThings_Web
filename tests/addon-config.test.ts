...
describe("Home Assistant add-on metadata", () => {
  test("publishes room-clone-only name rewriting as version 0.1.100", () => {
    ...
    expect(config.version).toBe("0.1.100");
    expect(packageMetadata.version).toBe("0.1.100");
    expect(protocolMetadata.bridge_version).toBe("0.1.100");
    expect(protocolMetadata.protocol_version).toBe(4);
    expect(runtime).toContain('const bridgeVersion = "0.1.100";');
