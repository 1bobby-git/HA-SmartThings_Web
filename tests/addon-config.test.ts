...
describe("Home Assistant add-on metadata", () => {
  test("packages restored liveness recovery as version 0.1.149", () => {
    ...
    expect(config.version).toBe("0.1.149");
    expect(config.homeassistant_api).toBe(true);
    expect(packageMetadata.version).toBe("0.1.149");
    expect(protocolMetadata.bridge_version).toBe("0.1.149");
    expect(protocolMetadata.protocol_version).toBe(4);
    expect(runtime).toContain('const bridgeVersion = "0.1.149";');
