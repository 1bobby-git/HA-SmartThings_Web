...
describe("Home Assistant add-on metadata", () => {
  test("publishes stable push confirmation as version 0.1.57", () => {
    ...
    expect(config.version).toBe("0.1.57");
    expect(packageMetadata.version).toBe("0.1.57");
    expect(protocolMetadata.bridge_version).toBe("0.1.57");
    expect(protocolMetadata.protocol_version).toBe(4);
    expect(runtime).toContain('const bridgeVersion = "0.1.57";');
    expect(changelog).toContain("## 0.1.57");
