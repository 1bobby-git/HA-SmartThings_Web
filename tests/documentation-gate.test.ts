...
describe("Phase 1 documentation gate", () => {
  test("keeps evidence-only decisions and real-account gaps explicit", () => {
    ...
    expect(serviceGuide).toContain("Home Assistant가 내부 Device Registry ID를 자동으로 전달합니다");
    expect(serviceGuide).toContain("응답에 나온 값을 그대로 복사");
    expect(serviceGuide).toContain("`identifier_*` 값을 `main`이나 raw `speechSynthesis`로 번역하지 않습니다");
    expect(serviceGuide).toContain("\"maxLength\": 1000");
    expect(serviceGuide).toContain("component default");
    expect(serviceGuide).toContain("device_not_found");
    expect(serviceGuide).toContain("device_ambiguous");
    expect(serviceGuide).toContain("device_offline");
    expect(serviceGuide).toContain("capability_not_found");
    expect(serviceGuide).toContain("command_confirmation_timeout");
    expect(addonDocs).toContain("https://github.com/1bobby-git/HA-SmartThings_Web/blob/main/docs/smartthings-web-services-ui-guide.md");
    expect(addonDocs).toContain("commands[].component");
